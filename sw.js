/* 英単語・例文暗記 スマホ版 — オフラインでも動くようにするための入れ物。
   版を上げたいときは CACHE の名前だけ変える（例 eiw-v1 → eiw-v2）。
   古い入れ物は activate のときに片づける。

   ★ 名前は作業時間管理（wta-*）と別にしてある。同じ github.io に置くので、
     入れ物の名前がぶつかると、片方の更新でもう片方の控えが消える。 */
const CACHE = "eiw-v1";
const FILES = [
  "./",
  "./index.html",
...
/* 初期単語.csv・基本3000語のCSV はここに入れない。
   置いていない人もいるので、addAll が 404 で丸ごと失敗してしまう
   （作業時間管理の 初期マスタ.csv と同じ落とし穴）。
   下の fetch（まず取りに行く形）で、読んだものが自然に控えに入る。 */
